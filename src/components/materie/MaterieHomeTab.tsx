...
import { Search, MessageSquare, Flame, BookOpen, TrendingUp, ChevronRight, Zap, RefreshCw } from 'lucide-react'
...
// Trending topics that rotate daily based on date seed
const ALL_TRENDING = [
  'Neue Weltordnung', 'Deep State', 'Chemtrails', '5G Netz', 'QAnon',
  'Bilderberg', 'Mondlandung', 'Sandy Hook', 'JFK Attentat', 'Operation Mockingbird',
  'Federal Reserve', 'Freimaurerloge', 'Illuminati', 'MK Ultra', 'HAARP',
  'COVID Ursprung', 'Impfkontroverse', 'UFO Disclosure', 'Agenda 21', 'Great Reset',
]

function getTodaysTrending(): string[] {
  const today = new Date()
  const seed = today.getFullYear() * 10000 + (today.getMonth() + 1) * 100 + today.getDate()
  const shuffled = [...ALL_TRENDING].sort((a, b) => {
    const ha = ((seed * a.charCodeAt(0)) % 97) - ((seed * b.charCodeAt(0)) % 97)
    return ha
  })
...
export function MaterieHomeTab() {
  ...
  const [trending] = useState<string[]>(getTodaysTrending())
    ...
    async function loadStats() {
      ...
      try {
        const [usersRes, msgsRes, articlesRes] = await Promise.all([
          ...
          supabase.from('articles').select('id', { count: 'exact', head: true }).eq('world', 'materie').eq('is_published', true),
        ])
        if (!cancelled) {
          setStats({
    ...
    <div className="px-4 py-4 space-y-6">
      ...
      >
        ...
        <div className="flex gap-4 mt-3">
          {statItems.map(s => (
            <div key={s.label}>
              <p className={`text-base font-black text-[#64B5F6] ${statsLoading ? 'animate-pulse' : ''}`}>{s.val}</p>
      ...
      {/* Trending Topics – tagesbasiert rotierend */}
      <div>
        <div className="flex items-center gap-2 mb-3">
          <TrendingUp size={14} className="text-[#FF5252]" />
          <h3 className="text-xs font-bold text-white/40 uppercase tracking-widest">Trending heute</h3>
        </div>
      ...
      <button
        ...
        className="w-full flex items-center gap-3 p-4 rounded-2xl border border-[#9C27B0]/30 bg-[#4A148C]/20 hover:bg-[#4A148C]/30 active:scale-[0.98] transition-all duration-200"
      >
