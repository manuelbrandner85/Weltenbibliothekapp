...
import { BookOpen, ChevronRight, Search, Tag, Loader2, RefreshCw } from 'lucide-react'
...
const CATEGORIES = [
  { id: 'geopolitik',   name: 'Geopolitik',     icon: '🌐', color: '#4CAF50' },
  { id: 'medien',       name: 'Medienkritik',   icon: '📺', color: '#FF5252' },
  { id: 'forschung',    name: 'Forschung',      icon: '🔬', color: '#9C27B0' },
  { id: 'finanzen',     name: 'Finanzsystem',   icon: '💰', color: '#FFD700' },
  { id: 'ueberwachung', name: 'Überwachung',    icon: '👁️', color: '#FF9800' },
  { id: 'gesundheit',   name: 'Gesundheit',     icon: '💊', color: '#E91E63' },
  { id: 'tech',         name: 'Technologie',    icon: '💻', color: '#00BCD4' },
  { id: 'geschichte',   name: 'Geschichte',     icon: '📜', color: '#8BC34A' },
]

const CATEGORY_COLORS: Record<string, string> = {
  geopolitik: '#4CAF50', medien: '#FF5252', forschung: '#9C27B0',
  finanzen: '#FFD700', ueberwachung: '#FF9800', gesundheit: '#E91E63',
  tech: '#00BCD4', geschichte: '#8BC34A', default: '#2196F3',
}
...
export default function MaterieWissenPage() {
  ...
  const loadArticles = useCallback(async (query: string, category: string | null) => {
    ...
    let q = supabase
      .from('articles')
      .select('id, title, excerpt, content, category, tags, image_url, view_count, like_count, published_at, is_published')
      .eq('world', 'materie')
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[10px] text-white/30">
                      {categoryCounts[cat.id] ?? categoryCounts[cat.id.toLowerCase()] ?? 0} Artikel
                    </p>
                  </div>
                  <ChevronRight
                    size={12}
                    className={`shrink-0 transition-colors ${activeCategory === cat.id ? 'text-[#64B5F6]' : 'text-white/20'}`}
                  />
                </button>
              ))}
            </div>
          ...
          <div>
            <div className="flex items-center gap-2 mb-3">
              ...
              {activeCategory && (
                <button
                  ...
                  className="ml-auto text-[10px] text-[#64B5F6] hover:text-white transition-colors"
                >
                  Filter entfernen
                </button>
              )}
            ...
            {loading ? (
              <div className="flex justify-center py-12">
                <Loader2 size={28} className="animate-spin text-[#2196F3]/60" />
              </div>
            ) : articles.length === 0 ? (
              <div className="rounded-2xl p-6 bg-[#1A1A1A]/60 border border-white/5 text-center">
                ...
                <p className="text-white/30 text-xs mt-1">Die Bibliothek wird kontinuierlich befüllt.</p>
              </div>
            ) : (
              <div className="space-y-3">
                {articles.map(article => (
                  <button
                    key={article.id}
                    onClick={() => router.push(`/materie/wissen/${article.id}`)}
