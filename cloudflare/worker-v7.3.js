/**
 * WELTENBIBLIOTHEK BACKEND v7.3
 * 
 * NEU:
 * - Telegram als Datenquelle (öffentliche Kanäle)
 * - Themen-spezifische Telegram-Kanäle
 * - Videos, PDFs, Dokumente aus Telegram
 * - Alternative Medien Integration
 */

// TELEGRAM KANÄLE & GRUPPEN (Öffentliche Quellen)
const TELEGRAM_CHANNELS = {
  // Allgemeine alternative Nachrichten
  general: [
    { name: 'RT DE', handle: 'rt_de', type: 'news', description: 'RT Deutsch - Alternative Nachrichtenquelle' },
    { name: 'Telegram News', handle: 'telegram', type: 'official', description: 'Offizieller Telegram Kanal' },
  ...
  // Dokumentationen
  documentaries: [
    { name: 'Documentary Hub', handle: 'documentaryhub', type: 'documentary', description: 'Dokumentarfilme und Analysen' },
    { name: 'Truth Seekers', handle: 'truthseekers', type: 'documentary', description: 'Wahrheitssucher-Dokumentationen' },
  ]
};
...
function generateMultimedia(query, narrative) {
  ...
  // Telegram-Ressourcen
  const telegramResources = getTelegramResources(query, narrative);
  ...
  return {
    ...
    telegram: telegramResources,  // 🆕 Telegram-Kanäle
    audio: []
  };
}
...
// TELEGRAM-RESSOURCEN GENERATOR
function getTelegramResources(query, narrative) {
  const queryLower = query.toLowerCase();
  let relevantChannels = [];
  
  // Themen-spezifische Kanäle
  if (queryLower.includes('mk ultra') || queryLower.includes('mkultra') || queryLower.includes('mind control')) {
    relevantChannels = [...TELEGRAM_CHANNELS.mind_control, ...TELEGRAM_CHANNELS.whistleblower];
  } else if (queryLower.includes('area 51') || queryLower.includes('ufo') || queryLower.includes('alien')) {
    relevantChannels = [...TELEGRAM_CHANNELS.ufo, ...TELEGRAM_CHANNELS.documentaries];
  } else if (queryLower.includes('9/11') || queryLower.includes('911')) {
    relevantChannels = [...TELEGRAM_CHANNELS.false_flags, ...TELEGRAM_CHANNELS.documentaries];
  } else if (queryLower.includes('illuminati') || queryLower.includes('geheimgesellschaft')) {
    relevantChannels = [...TELEGRAM_CHANNELS.conspiracy, ...TELEGRAM_CHANNELS.whistleblower];
  } else {
    // Fallback: Allgemeine Kanäle
    relevantChannels = [...TELEGRAM_CHANNELS.general, ...TELEGRAM_CHANNELS.documentaries];
  }
  
  return {
    channels: relevantChannels.map(channel => ({
      name: channel.name,
      handle: channel.handle,
      url: `https://t.me/${channel.handle}`,
      description: channel.description,
      type: channel.type,
      // Direkte Medien-Links (Beispiele)
      mediaLinks: {
        search: `https://t.me/s/${channel.handle}`,  // Web-Version für Suche
        bot: `https://t.me/${channel.handle}`,        // Direkter Kanal-Link
      }
    })),
