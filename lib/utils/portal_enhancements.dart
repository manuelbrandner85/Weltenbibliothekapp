...
class StarfieldPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    for (final star in stars) {
      ...
      final y =
          (star.y + progress * star.parallaxLayer * 0.1) % 1.0 * size.height;

      // Twinkle effect
      final twinkle = (math.sin(
                  animation * math.pi * 2 + star.twinkleOffset * math.pi * 2) +
              1) /
          2;
      ...
      if (star.size > 1.5) {
        final glowPaint = Paint()
          ..color =
              Colors.white.withValues(alpha: alpha * 0.2 * star.parallaxLayer)
