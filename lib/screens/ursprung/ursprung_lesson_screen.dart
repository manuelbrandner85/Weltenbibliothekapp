...
///
/// 5-Tab Lesson UI für ein einzelnes Modul (U-QC-XX).
...
class UrsprungLessonScreen extends StatefulWidget {
  final String moduleCode;

  const UrsprungLessonScreen({super.key, required this.moduleCode});

  @override
  State<UrsprungLessonScreen> createState() => _UrsprungLessonScreenState();
}

class _UrsprungLessonScreenState extends State<UrsprungLessonScreen> {
  ...
  Future<void> _fetchModule() async {
    ...
    try {
      ...
      final data = await UrsprungService.fetchModule(
        widget.moduleCode,
        userId: user?.id,
      );
      setState(() {
  ...
  Future<void> _submitProgress(
    ...
    try {
      final uri = Uri.parse('${ApiConfig.workerUrl}/api/ursprung/progress');
