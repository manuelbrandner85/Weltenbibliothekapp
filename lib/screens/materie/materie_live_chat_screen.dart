...
class _MaterieLiveChatScreenState extends State<MaterieLiveChatScreen>
  ...
  Future<void> _loadLiveSchedule() async {
    final sched =
        await LiveScheduleService.instance.load('materie', _selectedRoom);
    if (!mounted) return;
    setState(() {
  ...
  Future<void> _scheduleLiveSession() async {
    final backendRole = StorageService().getMaterieProfile()?.role;
    ...
    if (saved == false) {
      await LiveScheduleService.instance.clear('materie', _selectedRoom);
      if (!mounted) return;
      setState(() {
        _scheduledAt = null;
