...
class _DataLeakToolState extends State<DataLeakTool> {
  ...
  String _obfuscateLine(String line) {
    ...
    if (parts.length >= 2) {
      ...
      final stars = '••••${pass.length > 4 ? pass.substring(pass.length - 2) : ''}';
    ...
    return Scaffold(
      ...
      appBar: WBGlassAppBar(
        ...
        titleWidget: Row(children: [
          ...
          const Text('Datenleck-Prüfer', style: TextStyle(color: _kText, fontWeight: FontWeight.bold)),
        ]),
      ),
      body: SingleChildScrollView(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
          _card(Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            const Text('E-Mail-Adresse prüfen', style: TextStyle(color: _kMuted, fontSize: 12)),
          ...
          if (_result != null) ...[
            ...
            Container(
              ...
              padding: const EdgeInsets.all(16),
              decoration: BoxDecoration(
                color: count == 0 ? _kGreen.withValues(alpha: 0.1) : _kAccent.withValues(alpha: 0.1),
                borderRadius: BorderRadius.circular(12),
                border: Border.all(color: count == 0 ? _kGreen.withValues(alpha: 0.4) : _kAccent.withValues(alpha: 0.4)),
              ),
              child: Row(children: [
                Icon(
                  count == 0 ? Icons.check_circle_rounded : Icons.warning_rounded,
                ...
                Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Text(
                    count == 0 ? 'Keine Treffer gefunden' : '$count Einträge gefunden',
                    style: TextStyle(
                      color: count == 0 ? _kGreen : _kAccent,
            ...
            if (lines.isNotEmpty)
              _card(Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Row(children: [
                  ...
                  const Text('Beispiel-Einträge (Passwörter verschleiert)', style: TextStyle(color: _kMuted, fontSize: 12)),
                ]),
                const SizedBox(height: 8),
            ...
            if (count > 0)
              _card(Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Row(children: [
                  Icon(Icons.tips_and_updates_rounded, color: _kAmber, size: 16),
                  const SizedBox(width: 6),
                  const Text('Empfehlungen', style: TextStyle(color: _kAmber, fontWeight: FontWeight.bold, fontSize: 13)),
                ]),
                const SizedBox(height: 8),
                const Text(
