...
class _ImageAnalysisToolState extends State<ImageAnalysisTool> {
            ...
            _card(
              ...
              Text('Datei-Informationen',
                  style: TextStyle(
                      ...
                      fontSize: 14)),
              const SizedBox(height: 8),
              if (_result!['contentType'] != null)
                _row('Content-Type', _result!['contentType'].toString()),
              if (_result!['size'] != null)
                _row('Dateigröße', _result!['size'].toString()),
              if (_result!['width'] != null && _result!['height'] != null)
                _row('Abmessungen',
                    '${_result!['width']} × ${_result!['height']} px'),
              if (_result!['lastModified'] != null)
                _row('Geändert', _result!['lastModified'].toString()),
            ])),

            if (_result!['exif'] != null &&
                (_result!['exif'] as Map).isNotEmpty)
              _card(Column(
                  ...
                  children: [
                    ...
                    ...(_result!['exif'] as Map)
                        .entries
                        .take(15)
                        .map((e) => _row(e.key.toString(), e.value.toString())),
                  ])),

            // Suchen-Buttons
            _card(
                Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              const Text('Suchen in',
                  style: TextStyle(color: _kMuted, fontSize: 12)),
              const SizedBox(height: 10),
              Row(children: [
                _searchBtn('TinEye', Icons.image_search, () {
                  _openUrl(
                      'https://tineye.com/search?url=${Uri.encodeComponent(_urlCtrl.text.trim())}');
                }),
                const SizedBox(width: 8),
                _searchBtn('Google', Icons.search, () {
                  _openUrl(
                      'https://www.google.com/searchbyimage?image_url=${Uri.encodeComponent(_urlCtrl.text.trim())}');
                }),
                const SizedBox(width: 8),
                _searchBtn('Yandex', Icons.travel_explore, () {
                  _openUrl(
                      'https://yandex.com/images/search?rpt=imageview&url=${Uri.encodeComponent(_urlCtrl.text.trim())}');
                }),
              ]),
              const SizedBox(height: 10),
              OutlinedButton.icon(
  ...
  Widget _searchBtn(String label, IconData icon, VoidCallback onTap) =>
      Expanded(
