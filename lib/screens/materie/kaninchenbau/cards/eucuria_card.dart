  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          const Icon(Icons.policy_rounded, color: _accent, size: 18),
        ...
        if (loading)
          ...
          Padding(padding: const EdgeInsets.all(20), child: Text('Keine EU-Rechtsartikel zum Thema gefunden.', style: TextStyle(color: Colors.white.withValues(alpha: 0.4))))
        else
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Wrap(spacing: 6, runSpacing: 4, children: [
              if (c.author.isNotEmpty)
                Text(c.author, style: TextStyle(color: Colors.white.withValues(alpha: 0.55), fontSize: 10)),
              if (c.journal.isNotEmpty)
                Text(c.journal, style: TextStyle(color: Colors.white.withValues(alpha: 0.4), fontSize: 10), maxLines: 1, overflow: TextOverflow.ellipsis),
              if (c.year.isNotEmpty)
                Text(c.year, style: TextStyle(color: Colors.white.withValues(alpha: 0.3), fontSize: 10)),
              if (c.doi.isNotEmpty)
                Container(padding: const EdgeInsets.symmetric(horizontal: 5, vertical: 2), decoration: BoxDecoration(color: _accent.withValues(alpha: 0.16), borderRadius: BorderRadius.circular(4)), child: const Text('DOI', style: TextStyle(color: _accent, fontSize: 9, fontWeight: FontWeight.w700))),
            ]),
          ]),
        ),
      ),
