...
class OpenSanctionsCard extends StatelessWidget {
  ...
  const OpenSanctionsCard({super.key, required this.results, required this.loading});
      ...
      child: InkWell(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            ...
            borderRadius: BorderRadius.circular(10),
            border: Border.all(color: _accent.withValues(alpha: _borderOpacity(r.topics))),
          ),
  ...
  Widget _schemaBadge(String schema) => Container(
        ...
        child: Text(schema, style: const TextStyle(color: _accent, fontSize: 9, fontWeight: FontWeight.w700)),
      );

  Widget _topicBadge(String topic) {
    final isSanction = topic == 'sanction';
    final isPep = topic == 'pep';
    final color = isSanction ? _accent : isPep ? const Color(0xFFFFB300) : Colors.white54;
    return Container(
      padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 2),
      decoration: BoxDecoration(
        color: color.withValues(alpha: 0.16),
        borderRadius: BorderRadius.circular(4),
        border: isSanction ? Border.all(color: color.withValues(alpha: 0.4), width: 0.8) : null,
      ),
