  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          const Icon(Icons.groups_2_rounded, color: _accent, size: 18),
        ...
        if (loading)
          ...
          Padding(padding: const EdgeInsets.all(20), child: Text('Keine Vorstands-Verflechtungen zum Thema gefunden.', style: TextStyle(color: Colors.white.withValues(alpha: 0.4))))
        else
        ...
        child: Container(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(children: [
              ...
              const SizedBox(width: 8),
              Expanded(child: Text(o.officerName, style: const TextStyle(color: Colors.white, fontSize: 12, fontWeight: FontWeight.w600), maxLines: 1, overflow: TextOverflow.ellipsis)),
            ]),
            const SizedBox(height: 6),
            Wrap(spacing: 6, runSpacing: 4, children: [
              if (o.position.isNotEmpty)
                Container(padding: const EdgeInsets.symmetric(horizontal: 5, vertical: 2), decoration: BoxDecoration(color: _accent.withValues(alpha: 0.16), borderRadius: BorderRadius.circular(4)), child: Text(o.position, style: const TextStyle(color: _accent, fontSize: 9, fontWeight: FontWeight.w700), maxLines: 1, overflow: TextOverflow.ellipsis)),
              if (o.companyName.isNotEmpty)
                Text('@ ${o.companyName}', style: TextStyle(color: Colors.white.withValues(alpha: 0.55), fontSize: 10), maxLines: 1, overflow: TextOverflow.ellipsis),
              if (o.startDate.isNotEmpty)
                Text('seit ${o.startDate.length > 10 ? o.startDate.substring(0, 10) : o.startDate}', style: TextStyle(color: Colors.white.withValues(alpha: 0.3), fontSize: 10)),
            ]),
          ]),
        ),
      ),
