...
class DocumentCloudCard extends StatelessWidget {
  ...
  final bool loading;
  const DocumentCloudCard({super.key, required this.items, required this.loading});
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          const Icon(Icons.picture_as_pdf_rounded, color: _accent, size: 18),
        ...
        if (loading)
          ...
          Padding(padding: const EdgeInsets.all(20), child: Text('Keine Dokumente zum Thema gefunden.', style: TextStyle(color: Colors.white.withValues(alpha: 0.4))))
        else
    ...
    return Padding(
      ...
      child: InkWell(
        ...
        child: Container(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(children: [
              const Icon(Icons.description_rounded, color: _accent, size: 15),
              const SizedBox(width: 8),
            ...
            Wrap(spacing: 6, runSpacing: 4, children: [
              if (d.organization.isNotEmpty)
                Container(padding: const EdgeInsets.symmetric(horizontal: 5, vertical: 2), decoration: BoxDecoration(color: _accent.withValues(alpha: 0.16), borderRadius: BorderRadius.circular(4)), child: Text(d.organization, style: const TextStyle(color: _accent, fontSize: 9, fontWeight: FontWeight.w700), maxLines: 1, overflow: TextOverflow.ellipsis)),
              if (d.pageCount > 0)
                Text('${d.pageCount} S.', style: TextStyle(color: Colors.white.withValues(alpha: 0.4), fontSize: 10)),
              if (d.dateUploaded.isNotEmpty)
                Text(d.dateUploaded.length > 10 ? d.dateUploaded.substring(0, 10) : d.dateUploaded, style: TextStyle(color: Colors.white.withValues(alpha: 0.3), fontSize: 10)),
            ]),
          ]),
        ),
      ),
