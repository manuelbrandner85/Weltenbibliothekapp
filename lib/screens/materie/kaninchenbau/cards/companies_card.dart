...
class CompaniesCard extends StatelessWidget {
  ...
  const CompaniesCard({super.key, required this.companies, required this.loading});
  ...
  Widget _buildEntry(CompanyEntry c) {
    final statusColor = c.status.toLowerCase().contains('active')
        ? Colors.greenAccent
        : c.status.toLowerCase().contains('dissolv') || c.status.toLowerCase().contains('inactiv')
            ? Colors.redAccent
            : Colors.white38;
        ...
        child: Container(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Wrap(spacing: 6, runSpacing: 4, children: [
              ...
              if (c.lei != null)
                _chip('LEI: ${c.lei!.substring(0, 8)}…', const Color(0xFF7986CB)),
            ]),
  ...
  Widget _sourceBadge(String source) => Container(
        ...
        child: Text(source, style: const TextStyle(color: _accent, fontSize: 8, fontWeight: FontWeight.w800, letterSpacing: 0.8)),
      );
