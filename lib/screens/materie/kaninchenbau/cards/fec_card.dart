    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        if (loading)
          ...
          Padding(padding: const EdgeInsets.all(20), child: Text('Keine FEC-Kandidaten zum Thema gefunden.', style: TextStyle(color: Colors.white.withValues(alpha: 0.4))))
        else
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Wrap(spacing: 6, runSpacing: 4, children: [
              if (c.party.isNotEmpty)
                Container(padding: const EdgeInsets.symmetric(horizontal: 5, vertical: 2), decoration: BoxDecoration(color: _accent.withValues(alpha: 0.16), borderRadius: BorderRadius.circular(4)), child: Text(c.party, style: const TextStyle(color: _accent, fontSize: 9, fontWeight: FontWeight.w700))),
              if (c.office.isNotEmpty)
                Text(c.office, style: TextStyle(color: Colors.white.withValues(alpha: 0.55), fontSize: 10)),
              if (c.state.isNotEmpty)
                Text(c.state + (c.district.isNotEmpty ? ' · ${c.district}' : ''), style: TextStyle(color: Colors.white.withValues(alpha: 0.4), fontSize: 10)),
            ]),
          ]),
        ),
      ),
