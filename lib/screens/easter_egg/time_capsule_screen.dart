...
class _TimeCapsuleScreenState extends State<TimeCapsuleScreen>
  ...
  @override
  void dispose() { _msgCtrl.dispose(); _ambientCtrl.dispose(); super.dispose(); }

  Future<void> _load() async {
    final prefs = await SharedPreferences.getInstance();
    _capsules = (prefs.getStringList(_kKey) ?? const [])
        .map((s) { try { return _Capsule.fromJson(jsonDecode(s) as Map<String, dynamic>); } catch (_) { return null; } })
        .whereType<_Capsule>()
        .toList()
  ...
  Future<void> _create() async {
    ...
    final prefs = await SharedPreferences.getInstance();
    await prefs.setStringList(_kKey, _capsules.map((c) => jsonEncode(c.toJson())).toList());
    _msgCtrl.clear();
    setState(() {});
    if (!mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
      content: Text('⏳ Versiegelt. Öffnet sich am ${_fmt(_capsules.last.openAt)}'),
  ...
  Future<void> _open(_Capsule c) async {
    ...
    _capsules[idx] = _Capsule(
      id: c.id, message: c.message, sealedAt: c.sealedAt, openAt: c.openAt, opened: true,
    );
    final prefs = await SharedPreferences.getInstance();
    await prefs.setStringList(_kKey, _capsules.map((x) => jsonEncode(x.toJson())).toList());
    setState(() {});
    if (!mounted) return;
    showDialog(context: context, builder: (_) => AlertDialog(
      ...
      title: const Row(children: [
        ...
        SizedBox(width: 8),
        Text('NACHRICHT VON FRÜHER', style: TextStyle(color: _gold, fontSize: 13, letterSpacing: 2)),
      ]),
      content: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text('Versiegelt am ${_fmt(c.sealedAt)} (vor ${DateTime.now().difference(c.sealedAt).inDays}d)',
            style: const TextStyle(color: Colors.white54, fontSize: 11)),
        const SizedBox(height: 14),
        Text(c.message, style: const TextStyle(color: Colors.white, fontSize: 15, height: 1.6)),
      ]),
      actions: [TextButton(onPressed: () => Navigator.pop(context), child: const Text('Schließen', style: TextStyle(color: _primary)))],
    ));
  }
        ...
        SafeArea(child: ListView(padding: const EdgeInsets.fromLTRB(14, 8, 14, 28), children: [
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                TextField(
                  ...
                  decoration: InputDecoration(
                    hintText: 'Was möchtest du deinem zukünftigen Selbst sagen?',
                ...
                Row(children: [
                  const Text('Öffnen in:', style: TextStyle(color: Colors.white60, fontSize: 11)),
                  ...
                  ...[1, 3, 6, 12].map((m) {
                    ...
                    return Padding(
                      ...
                      child: GestureDetector(
                        onTap: () { HapticFeedback.lightImpact(); setState(() => _months = m); },
                        child: Container(
                          ...
                          child: Text('${m}M', style: TextStyle(color: sel ? Colors.white : Colors.white60, fontSize: 11, fontWeight: FontWeight.w600)),
                        ),
                      ),
                    );
                  }),
                ...
                SizedBox(
                  ...
                  child: ElevatedButton.icon(
                    ...
                    label: const Text('VERSIEGELN', style: TextStyle(fontWeight: FontWeight.bold, letterSpacing: 1.5)),
          ...
          if (pending.isNotEmpty) ...[
            ...
            Text('⏳ VERSIEGELT (${pending.length})',
                style: TextStyle(color: _primary.withValues(alpha: 0.8), fontSize: 10, letterSpacing: 2, fontWeight: FontWeight.w700)),
            const SizedBox(height: 6),
          ...
          if (opened.isNotEmpty) ...[
            ...
            Text('📂 GEÖFFNET (${opened.length})',
                style: const TextStyle(color: Colors.white38, fontSize: 10, letterSpacing: 2, fontWeight: FontWeight.w700)),
            const SizedBox(height: 6),
            ...opened.map((c) => _capsuleCard(c, ripe: false, opened: true)),
          ],
        ])),
    ...
    return Padding(
      ...
      child: Material(color: Colors.transparent, child: InkWell(
        ...
        child: Container(
          ...
          child: Row(children: [
            Icon(ripe ? Icons.mark_email_unread_rounded : (opened ? Icons.drafts_rounded : Icons.lock_rounded),
                color: col, size: 22),
            const SizedBox(width: 10),
            Expanded(
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text(opened ? c.message : (ripe ? 'BEREIT — Tippe zum Öffnen' : 'Versiegelte Nachricht'),
                    style: TextStyle(color: opened ? Colors.white60 : col, fontSize: 12, fontWeight: FontWeight.w600),
                    maxLines: 1, overflow: TextOverflow.ellipsis),
                const SizedBox(height: 2),
                Text(ripe ? 'fällig: ${_fmt(c.openAt)}'
                        : opened ? 'geöffnet am ${_fmt(c.openAt)}'
...
class _Capsule {
  ...
  final bool opened;
  const _Capsule({required this.id, required this.message, required this.sealedAt, required this.openAt, required this.opened});
  Map<String, dynamic> toJson() => {'id': id, 'msg': message, 'sealed': sealedAt.toIso8601String(), 'open': openAt.toIso8601String(), 'opened': opened};
  factory _Capsule.fromJson(Map<String, dynamic> j) => _Capsule(
    id: j['id'] as String? ?? '',
    message: j['msg'] as String? ?? '',
    sealedAt: DateTime.tryParse(j['sealed'] as String? ?? '') ?? DateTime.now(),
...
class _TcOrbsPainter extends CustomPainter {
  ...
  @override void paint(Canvas canvas, Size size) {
    _d(canvas, Offset(size.width * 0.2, size.height * (0.3 + math.sin(t * 2 * math.pi) * 0.05)), 100, const Color(0xFF42A5F5));
  ...
  @override bool shouldRepaint(_TcOrbsPainter o) => o.t != t;
}
