...
class _VorhangModulesScreenState extends State<VorhangModulesScreen> {
  ...
  Future<void> _detectNewUnlocks(
    ...
    for (final code in unlockedCodes) {
      if (await NewUnlockTracker.instance.isNew('vorhang', code)) {
        fresh.add(code);
      }
    }
    await NewUnlockTracker.instance.markSeen('vorhang', unlockedCodes);
  ...
  Future<void> _submit() async {
    ...
    if (result.passed && mounted) {
      ...
      try {
        ...
        if (kDebugMode) {
          debugPrint('vorhang_modules_screen: XP-Sync fehlgeschlagen -> $e');
        }
