...
class _AkashaChronicleScreenState extends State<AkashaChronicleScreen>
  ...
  Future<void> _addEntry() async {
    ...
    try {
      ...
      await SpiritReadingService.instance.save(
        ...
        tool: 'akasha',
        summary:
            '${_selectedCat.emoji} ${_selectedCat.label} · Mood $_selectedMood',
        result: {
          'category': _selectedCat.code,
          'mood': _selectedMood,
          'text': text,
    ...
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
      content: Text(
          '🌌 Eintrag in Akasha-Chronik gespeichert · Streak $_currentStreak'),
  ...
  Future<void> _requestReflection(_JournalEntry entry) async {
    ...
    try {
      final prompt = StringBuffer()
        ..writeln(
            'Reflektiere folgenden Tagebuch-Eintrag aus spiritueller Sicht:')
        ..writeln(
            'Kategorie: ${_categories.firstWhere((c) => c.code == entry.category, orElse: () => _categories[0]).label}')
      ...
      setState(() {
        ...
        if (i >= 0) {
          _entries[i] = _entries[i]
              .copyWith(reflection: reflection, reflectionLoading: false);
        }
      });
      await _persist();
    } catch (e) {
                ...
                _metric(
                    ...
                    entriesTotal == 0
                        ...
                        : (last30
                                    .where((e) => e > 0)
                                    .fold<int>(0, (s, e) => s + e) /
                                math.max(1, last30.where((e) => e > 0).length))
                            .toStringAsFixed(1),
                    '/ 5'),
              ]),
                  ...
                  return GestureDetector(
                    ...
                    child: AnimatedContainer(
                      ...
                      decoration: BoxDecoration(
                        ...
                        color: sel
                            ? _moodColor(m).withValues(alpha: 0.3)
                            : Colors.white.withValues(alpha: 0.05),
                        border: Border.all(
                            color: sel ? _moodColor(m) : Colors.transparent,
                            width: 2),
                      ),
                      child: Center(
          ...
          child: SizedBox(
            ...
            child: ListView(
              ...
              children: [
                ...
                ..._categories
                    .map((c) => _filterPill('${c.emoji} ${c.label}', c.code)),
              ],
            ),
          ),
        ),
  ...
  Widget _entryCard(_JournalEntry entry) {
    final cat = _categories.firstWhere((c) => c.code == entry.category,
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          if (entry.reflection == null)
            ListTile(
              ...
              title: const Text('AI-Reflexion holen',
                  style: TextStyle(color: Colors.white)),
              onTap: () {
                Navigator.pop(ctx);
                _requestReflection(entry);
              },
            ),
          ListTile(
            leading: const Icon(Icons.copy_rounded, color: Colors.white70),
            title: const Text('In Zwischenablage',
                style: TextStyle(color: Colors.white)),
            onTap: () {
          ...
          ListTile(
            ...
            title: const Text('Lokal löschen',
...
class _JournalEntry {
  ...
  _JournalEntry copyWith({String? reflection, bool? reflectionLoading}) =>
      _JournalEntry(
        id: id,
        category: category,
        mood: mood,
