...
class _EnergieLiveChatScreenState extends State<EnergieLiveChatScreen> {
  ...
  Future<void> _editMessage(String messageId, String currentText) async {
    // 🔍 DEBUG: Edit-Aufruf loggen
    if (kDebugMode) {
      debugPrint('🔍 Energie _editMessage aufgerufen');
    ...
    if (result != null && result.isNotEmpty && result != currentText) {
      try {
        // ✅ FIX 15: Verwende API-Methode (konsistent mit MATERIE)
  ...
  Future<void> _deleteMessage(Map<String, dynamic> msg) async {
    ...
    try {
      ...
      await _api.deleteChatMessage(
        ...
        userId: _userId,
        username: _username,
        realm: 'energie',  // 🔧 FIX: Correct realm for ENERGIE
      );
      ...
      if (kDebugMode) {
        debugPrint('❌ ENERGIE Delete failed: $e');
      }
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
