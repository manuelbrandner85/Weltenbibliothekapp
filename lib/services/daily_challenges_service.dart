...
class DailyChallengesService {
  ...
  Future<void> _loadChallenges() async {
    try {
      ...
      if (challengesJson != null) {
        ...
        _todaysChallenges =
            decoded.map((json) => DailyChallenge.fromJson(json)).toList();
      } else {
  ...
  Future<void> incrementProgress(ChallengeCategory category,
      {int amount = 1}) async {
    try {
      ...
      for (int i = 0; i < _todaysChallenges.length; i++) {
        ...
        if (challenge.category == category && !challenge.isCompleted) {
          ...
          _todaysChallenges[i] =
              challenge.copyWith(currentProgress: newProgress);
          ...
          if (!challenge.isCompleted && _todaysChallenges[i].isCompleted) {
            ...
            await AchievementService().incrementProgress(
                'first_search'); // Trigger achievement als Belohnung

            if (kDebugMode) {
              debugPrint(
                  '🎉 Challenge completed: ${_todaysChallenges[i].title}');
