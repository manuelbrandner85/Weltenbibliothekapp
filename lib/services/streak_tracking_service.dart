...
class StreakTrackingService {
  static final StreakTrackingService _instance =
  ...
  Future<void> _loadStreakData() async {
    try {
      ...
      _loginHistory = historyStrings.map((s) => DateTime.parse(s)).toList()
  ...
  Future<void> _saveStreakData() async {
    try {
      ...
      final historyStrings =
          _loginHistory.take(365).map((d) => d.toIso8601String()).toList();
  ...
  Future<void> trackDailyLogin() async {
    ...
    if ([7, 14, 30, 60, 90, 180, 365].contains(_currentStreak)) {
      ...
      if (kDebugMode) {
        debugPrint(
            '🎉 Streak Milestone! $_currentStreak days (+$_currentStreak Bonus-Punkte)');
      }
    }
