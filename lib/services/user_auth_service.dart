...
class UserAuthService {
  ...
  static Future<String?> getUsername({String? world}) async {
    ...
    if (currentWorld != null) {
      final worldKey =
          currentWorld == 'materie' ? _keyMaterieUsername : _keyEnergieUsername;
      final worldUsername = prefs.getString(worldKey);
      if (worldUsername != null && worldUsername.isNotEmpty) {
        if (kDebugMode) {
          debugPrint(
              '✅ UserAuthService: Username für current world $currentWorld = $worldUsername');
        }
        return worldUsername;
      }
    }
  ...
  static Future<String?> getUserId({String? world}) async {
    ...
    if (world != null) {
      final worldKey =
          world == 'materie' ? _keyMaterieUserId : _keyEnergieUserId;
    ...
    if (currentWorld != null) {
      final worldKey =
          currentWorld == 'materie' ? _keyMaterieUserId : _keyEnergieUserId;
      final worldUserId = prefs.getString(worldKey);
      if (worldUserId != null && worldUserId.isNotEmpty) {
        if (kDebugMode) {
          debugPrint(
              '✅ UserAuthService: User ID für current world $currentWorld = $worldUserId');
        }
        return worldUserId;
      }
    }
  ...
  static Future<bool> isAuthenticated({String? world}) async {
    ...
    if (kDebugMode) {
      debugPrint(
          '✅ UserAuthService: isAuthenticated = $isAuth (world: ${world ?? 'current'})');
    }
  ...
  static Future<void> setUsername(String username, {String? world}) async {
    ...
    if (world != null) {
      ...
      if (kDebugMode) {
        debugPrint(
            '✅ UserAuthService: Username gesetzt = $username (world: $world)');
      }
    }
  }
  ...
  static Future<void> setUserId(String userId, {String? world}) async {
    ...
    if (world != null) {
      ...
      if (kDebugMode) {
        debugPrint(
            '✅ UserAuthService: User ID gesetzt = $userId (world: $world)');
      }
    }
  }
