...
class AISearchSuggestionService {
  ...
  // Singleton
  static final AISearchSuggestionService _instance =
  ...
  Future<List<String>> getSmartSuggestions() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final suggestions =
            (data['suggestions'] as List?)?.map((s) => s.toString()).toList() ??
                [];
  ...
  Future<Map<String, dynamic>> _analyzeUserInterests() async {
    ...
    final topKeywords = sortedKeywords.take(10).map((e) => e.key).toList();
  ...
  Future<List<Map<String, dynamic>>> getSimilarNarratives(
      String narrativeId) async {
    try {
      final response = await http
          .get(
  ...
  Future<List<String>> getQuerySuggestions(String partialQuery) async {
    ...
    try {
      final response = await http
          .get(
            Uri.parse(
                '$_backendUrl/api/ai/autocomplete?q=${Uri.encodeComponent(partialQuery)}'),
          )
    ...
    return historyEntries
        .where((entry) =>
            entry.query.toLowerCase().contains(partialQuery.toLowerCase()))
  ...
  Future<List<String>> getTrendingSearches() async {
    try {
      ...
      if (response.statusCode == 200) {
        final data = jsonDecode(response.body);
        final trending =
            (data['trending'] as List?)?.map((t) => t.toString()).toList() ??
                [];
