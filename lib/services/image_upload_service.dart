...
import 'dart:io'
    if (dart.library.html) '../stubs/dart_io_stub.dart'; // ✅ SocketException
...
class ImageUploadService {
  ...
  Future<String> uploadProfileImage({
    ...
    try {
      ...
      if (kDebugMode) {
        debugPrint(
            '📦 Image size: ${bytes.length} bytes (${(bytes.length / 1024).toStringAsFixed(2)} KB)');
      }
      ...
      final streamedResponse = await request.send().timeout(
        ...
        onTimeout: () {
          throw Exception(
              'Upload timeout after 60 seconds - please check your connection');
        },
      );
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        final imageUrl =
            data['media_url'] as String?; // ⚠️ Worker gibt 'media_url' zurück
  ...
  Future<String?> pickAndUploadImage(
      {String? userId, String? profileType}) async {
    try {
