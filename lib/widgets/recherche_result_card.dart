...
class RechercheResultCard extends StatefulWidget {
  ...
  @override
  State<RechercheResultCard> createState() => _RechercheResultCardState();
}

class _RechercheResultCardState extends State<RechercheResultCard> {
  // 🔧 FIX: Cache parsed data to prevent redundant parsing on every build
  late final Map<String, dynamic> _structured;
  late final String _inhalt;
  late final bool _isFallback;
  late final Map<String, String> _sections;
  
  @override
  void initState() {
    super.initState();
    
    // Parse data once in initState instead of on every build
    _structured = widget.analyseData['structured'] as Map<String, dynamic>? ?? {};
    _inhalt = widget.analyseData['inhalt'] as String? ?? '';
    _isFallback = widget.analyseData['isFallback'] as bool? ?? false;
    
    // Pre-parse all sections
    _sections = {
      'fakten': _extractFromInhalt(_inhalt, ['FAKTEN', '📌', 'Kernfakten', 'Wichtigste', 'Haupterkenntnisse']),
      'quellen': _extractFromInhalt(_inhalt, ['QUELLEN', 'Quelle:']),
      'analyse': _extractFromInhalt(_inhalt, ['ANALYSE', 'INTERPRETATION', 'Offiziell']),
      'alternative': _extractFromInhalt(_inhalt, ['ALTERNATIVE', 'KRITISCH', 'Systemkritisch']),
    };
  }
  
  // Helper method for section extraction
  String _extractFromInhalt(String inhalt, List<String> keywords) {
    if (inhalt.isEmpty) return '';
    
    for (final keyword in keywords) {
      final index = inhalt.indexOf(keyword);
      if (index != -1) {
        final start = index + keyword.length;
        var end = inhalt.indexOf('\n\n', start);
        if (end == -1) end = inhalt.length;
        
        final section = inhalt.substring(start, end).trim();
  ...
  Widget build(BuildContext context) {
    ...
    // 🔧 FIX: Use cached sections instead of re-parsing
    final fakten = _sections['fakten'] ?? '';
    final quellen = _sections['quellen'] ?? '';
    final analyse = _sections['analyse'] ?? '';
    final alternativeSicht = _sections['alternative'] ?? '';
      ...
      child: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            // ━━━━━━━━━━━━━━━━━━
            _buildTitleSection(context, widget.query, _isFallback),
            ...
            if (quellen.isNotEmpty) ...[
              _buildQuellenSectionMitBewertung(context, _structured),
              context.vSpaceLg,
            ],
