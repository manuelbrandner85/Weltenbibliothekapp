...
class _WorldSpecificUserManagementWidgetState
  ...
  Future<void> _loadUsers() async {
    ...
    try {
      // 🔥 WELTSPEZIFISCH: world korrekt übergeben
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('❌ Fehler beim Laden der User: $e'),
  ...
  Future<void> _promoteUser(String username) async {
    ...
    final confirm = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text('Möchtest du $username zu Admin befördern?'),
        actions: [
          TextButton(
    ...
    if (mounted) {
      if (success) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('✅ $username wurde zu Admin befördert'),
  ...
  Future<void> _demoteUser(String username) async {
    ...
    final confirm = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text('Möchtest du $username zum normalen User machen?'),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(context, false),
            child: const Text('Abbrechen'),
  ...
  Future<void> _deleteUser(String username) async {
    ...
    final confirm = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            Text('Möchtest du $username wirklich löschen?'),
    ...
    if (mounted) {
      if (success) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('✅ $username wurde gelöscht'),
            backgroundColor: Colors.green,
          ),
        );
        await _loadUsers(); // Refresh
  ...
  Widget build(BuildContext context) {
    ...
    // 🔥 WELTSPEZIFISCH: Keine User in dieser Welt
    if (_users.isEmpty) {
      return Center(
        child: Column(
    ...
    // 🔥 USER-LISTE MIT WELTSPEZIFISCHEN ACTIONS
      ...
      itemBuilder: (context, index) {
        ...
        return Card(
          ...
          child: ListTile(
            ...
            // Role
            subtitle: Text('${user.role} • ${widget.world}'),
            
            // 🔥 ACTIONS (nur für Root-Admin)
            trailing: Row(
              mainAxisSize: MainAxisSize.min,
              children: [
                // 🔥 PROMOTE: Root-Admin kann User zu Admin machen
                if (admin.isRootAdmin && user.role == 'user')
                  IconButton(
                    icon: const Icon(Icons.arrow_upward, color: Colors.green),
                    onPressed: () => _promoteUser(user.username),
                    tooltip: 'Zum Admin befördern',
                  ),
                
                // 🔥 DEMOTE: Root-Admin kann Admin zu User machen
                ...
                // 🔥 DELETE: Root-Admin kann User löschen
...
/// 🎯 EXAMPLE SCREEN - VERWENDUNG DES WIDGETS
/// 
          ...
          : const Center(
              child: Column(
                ...
                children: [
                  ...
                  Text('Diese Seite ist nur für Admins.'),
                ],
              ),
            ),
    );
