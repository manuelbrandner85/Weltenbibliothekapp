...
# Connections Board Enhanced
cat > lib/widgets/inline_tools/connections_board_enhanced.dart << 'CONNEOF'
  ...
  Future<void> _loadConnections() async {
    try {
      final response = await http.get(
        Uri.parse('https://weltenbibliothek-api.brandy13062.workers.dev/api/tools/connections?room_id=${widget.roomId}'),
  ...
  Future<void> _addConnection() async {
    ...
    try {
      final response = await http.post(
        Uri.parse('https://weltenbibliothek-api.brandy13062.workers.dev/api/tools/connections'),
        headers: {
...
echo "✅ Connections Board Enhanced erstellt"
