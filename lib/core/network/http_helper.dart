...
class HttpHelper {
    ...
    while (true) {
      ...
      try {
        if (kDebugMode && attempt > 1) {
          debugPrint(
              '🔄 [HTTP] Retry attempt $attempt/$maxRetries for $operation');
        }
        ...
        if (!isRetryable) {
          if (kDebugMode) {
            debugPrint(
                '⚠️ [HTTP] Non-retryable error (${result.statusCode}): $operation');
          }
          return result;
        }
  ...
  static HttpResult<Map<String, dynamic>> _parseResponse(
    ...
    if (kDebugMode) {
      debugPrint(
          '📡 [HTTP] ${response.request?.method} ${response.statusCode} $url');
    }
