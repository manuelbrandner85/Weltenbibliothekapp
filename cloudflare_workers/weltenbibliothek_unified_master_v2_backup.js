...
async function handleChat(request, path, env) {
  // GET /chat-rooms - Get all chat rooms from D1
  ...
  // GET /messages/:roomId - Get messages from D1
...
async function handleLiveRooms(request, path, env) {
  ...
  // GET /api/live/rooms - Get all active live rooms from D1
  if (path === '/api/live/rooms' && method === 'GET') {
    try {
      const { results } = await env.DB.prepare(
        "SELECT * FROM live_rooms WHERE status = 'live' ORDER BY created_at DESC"
      ).all();
  ...
  // POST /api/live/rooms - Create new live room in D1
  if (path === '/api/live/rooms' && method === 'POST') {
    try {
      ...
      const authHeader = request.headers.get('Authorization') || '';
      const token = authHeader.substring(7);
      const username = token.split('_')[2] || 'unknown';

      const roomId = 'room_' + Date.now();
      const now = Math.floor(Date.now() / 1000);

      await env.DB.prepare(
        'INSERT INTO live_rooms (room_id, chat_room_id, title, description, host_username, status, category, created_at, started_at, participant_count) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)'
      ).bind(
        roomId,
        chatRoomId,
        title,
