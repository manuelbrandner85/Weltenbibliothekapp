...
/// Gespeicherte Artikel Screen - Alle offline verfügbaren Artikel
...
class _SavedArticlesScreenState extends State<SavedArticlesScreen> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('💾 Gespeicherte Artikel'),
        actions: [
          // Speicherplatz-Info
          IconButton(
  ...
  void _deleteArticle(String articleId) async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        backgroundColor: const Color(0xFF1A1A1A),
        title: const Text('Artikel löschen?', style: TextStyle(color: Colors.white)),
        content: const Text(
          'Dieser Artikel wird offline nicht mehr verfügbar sein.',
  ...
  void _confirmDeleteAll() async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        backgroundColor: const Color(0xFF1A1A1A),
        title: const Text('Alle Artikel löschen?', style: TextStyle(color: Colors.white)),
        content: const Text(
          'WARNUNG: Alle gespeicherten Artikel werden gelöscht!',
        ...
        actions: [
          ...
          TextButton(
            onPressed: () => Navigator.pop(context, true),
            child: const Text('ALLE LÖSCHEN', style: TextStyle(color: Colors.red, fontWeight: FontWeight.bold)),
          ),
        ],
      ),
    );
    ...
    if (confirmed == true) {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Alle Artikel gelöscht')),
        );
      }
    }
  }
  ...
  void _cleanOldArticles() async {
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('$deletedCount alte Artikel gelöscht')),
      );
    }
  }
  ...
  void _showStorageInfo() async {
    ...
    if (mounted) {
      showDialog(
        ...
        builder: (context) => AlertDialog(
          backgroundColor: const Color(0xFF1A1A1A),
          title: const Text('💾 Speicherplatz', style: TextStyle(color: Colors.white)),
          content: Column(
            mainAxisSize: MainAxisSize.min,
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              ...
              _buildInfoRow('Geschätzter Speicher', '~${info['estimatedSizeMB']} MB'),
            ],
          ),
          actions: [
            TextButton(
